///////////////////////////////////////////////////////////////////////////////
// hungarian_algorithm.cpp: Implementation file for Class HungarianAlgorithm.
//
...
double HungarianAlgorithm::Solve(
  ...
  // In the solving functions matrices are seen to be saved MATLAB-internally
  // in row-order. (i.e. the matrix [1 2; 3 4] will be stored as a vector
  // [1 3 2 4], NOT [1 2 3 4]).
...
void HungarianAlgorithm::assignmentoptimal(
    int *assignment, double *cost, double *distMatrixIn, int nOfRows,
    int nOfColumns) {
  ...
  *cost = 0;
  ...
  for (row = 0; row<nOfElements; row++) {
    ...
    CHECK_GE(value, 0) << "All matrix elements have to be non-negative.";
  ...
  if (nOfRows <= nOfColumns) {
    ...
    for (row = 0; row<nOfRows; row++)
        for (col = 0; col<nOfColumns; col++)
            if (std::abs(distMatrix[row + nOfRows*col]) <
...
void HungarianAlgorithm::buildassignmentvector(
    int *assignment, bool *starMatrix, int nOfRows, int nOfColumns) {
  int row, col;
  ...
  while (zerosFound) {
    ...
    for (col = 0; col<nOfColumns; col++)
        if (!coveredColumns[col])
            for (row = 0; row<nOfRows; row++)
                if ((!coveredRows[row]) &&
                    (std::abs(distMatrix[row + nOfRows*col]) <
                     std::numeric_limits<double>::epsilon())) {
                  /* prime zero */
