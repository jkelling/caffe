...
namespace caffe {
  ...
  class TiledPredictTest3dIsotropic : public MultiDeviceTest<TypeParam> {
    typedef typename TypeParam::Dtype Dtype;

  protected:
    TiledPredictTest3dIsotropic()
            : reference_output_() {
    ...
    virtual void SetUp() {
      ...
      std::stringstream md;
      md << "name: 'sample_net_tiled_predict_3d_isotropic'" << std::endl;
      ...
      // Setup sample solver
      std::stringstream sd;
      sd << "net: 'sample_net_tiled_predict_3d_isotropic-train.prototxt'"
         << std::endl;
      ...
      // Create modeldef.h5 file
      {
        ...
        ASSERT_GE(file_id, 0) << "Failed to create HDF5 file" <<
            modeldef_file_name_;
        hdf5_save_string(
            file_id, ".unet-ident", "sample_net_tiled_predict_3d_isotropic");
        H5Gcreate(file_id, "/unet_param", H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT);
        hdf5_save_string(
            file_id, "/unet_param/name",
            "sample_net_tiled_predict_3d_isotropic");
        hdf5_save_string(
      ...
      // conv_d0a-d1a <== average sub-sampling
      {
        ...
        ASSERT_EQ(2 * 2 * 2, w->count());
        for (int i = 0; i < w->count(); ++i)
            w->mutable_cpu_data()[i] = Dtype(0.125);
        shared_ptr< Blob<Dtype> > &b =
      ...
      // conv_d1a-d1b <== identity transform
      {
        ...
        ASSERT_EQ(3 * 3 * 3, w->count());
        std::memset(w->mutable_cpu_data(), 0, w->count() * sizeof(Dtype));
        w->mutable_cpu_data()[(3 * 3 * 3) / 2] = Dtype(1);
        shared_ptr< Blob<Dtype> > &b =
      ...
      // conv_d1a-d1b <== ch0: id, ch1: -id
      {
        ...
        ASSERT_EQ(2 * 9 * 9 * 9, w->count());
        std::memset(w->mutable_cpu_data(), 0, w->count() * sizeof(Dtype));
        w->mutable_cpu_data()[(9 * 9 * 9) / 2] = Dtype(1);
        w->mutable_cpu_data()[(9 * 9 * 9) / 2 + (9 * 9 * 9)] = Dtype(-1);
        shared_ptr< Blob<Dtype> > &b =
    ...
    virtual ~TiledPredictTest3dIsotropic() {
  ...
  template<typename TypeParam>
  void TiledPredictTest3dIsotropic<TypeParam>::CheckBlobEqual(
  ...
  // equal the reference
  template<typename TypeParam>
  void TiledPredictTest3dIsotropic<TypeParam>::TestTiledPredict(
      std::string const &param_name, std::string const &param_value,
      int iterations, bool average_mirror, bool average_rotate) {
  ...
  TYPED_TEST_CASE(TiledPredictTest3dIsotropic, TestDtypesAndDevices);

  TYPED_TEST(TiledPredictTest3dIsotropic, TestTiledPredictTileSize) {
    this->TestTiledPredict("tile_size", "20x20x20", 1, false, false);
  ...
  TYPED_TEST(TiledPredictTest3dIsotropic, TestTiledPredictNTilesZYX) {
  ...
  TYPED_TEST(TiledPredictTest3dIsotropic,
