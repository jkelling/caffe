...
void PoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  vector<int> spatial_dim_blob_shape(1, num_spatial_axes_);
  VLOG(1) << "  " << this->layer_param_.name()
          << " - Reshaping output_shape_ to "
          << toString(spatial_dim_blob_shape) << std::endl;
  output_shape_.Reshape(spatial_dim_blob_shape);
  int* output_shape_data = output_shape_.mutable_cpu_data();
  int pad_sum = 0;
  for (int i = 0; i < num_spatial_axes_; ++i) {
    float ocf = static_cast<float>(
        input_shape_data[i+1] + 2 * pad_data[i]
        - kernel_shape_data[i]) / stride_data[i];
    pad_sum += pad_data[i];
    output_shape_data[i] = static_cast<int>(
        (round_mode_ == PoolingParameter_RoundMode_FLOOR) ?
        std::floor(ocf) : std::ceil(ocf)) + 1;
  }
  if (pad_sum) {
    for (int i = 0; i < num_spatial_axes_; ++i) {
        ...
        if (num_spatial_axes_ == 2) {
          for (int ph = 0; ph < output_shape_data[0]; ++ph) {
            for (int pw = 0; pw < output_shape_data[1]; ++pw) {
              ...
              int hend = std::min(hstart + kernel_shape[0], input_shape_data[1]);
              int wend = std::min(wstart + kernel_shape[1], input_shape_data[2]);
          ...
          for (int ph = 0; ph < output_shape_data[0]; ++ph) {
            for (int pw = 0; pw < output_shape_data[1]; ++pw) {
              for (int pz = 0; pz< output_shape_data[2]; ++pz) {
                ...
                int hend = std::min(hstart + kernel_shape[0], input_shape_data[1]);
                int wend = std::min(wstart + kernel_shape[1], input_shape_data[2]);
                int zend = std::min(zstart + kernel_shape[2], input_shape_data[3]);
                hstart = std::max(hstart, 0);
                wstart = std::max(wstart, 0);
                zstart = std::max(zstart, 0);
                const int pool_index = (ph * output_shape_data[1] + pw)*
          ...
          if (num_spatial_axes_ == 2) {
            for (int ph = 0; ph < output_shape_data[0]; ++ph) {
              for (int pw = 0; pw < output_shape_data[1]; ++pw) {
                int hstart = ph * stride_data[0] - pad_data[0];
                int wstart = pw * stride_data[1] - pad_data[1];
                int hend = std::min(hstart + kernel_shape[0],
                         input_shape_data[1] + pad_data[0]);
                int wend = std::min(wstart + kernel_shape[1],
                         input_shape_data[2] + pad_data[1]);
                int pool_size = (hend - hstart) * (wend - wstart);
                hstart = std::max(hstart, 0);
                wstart = std::max(wstart, 0);
                hend = std::min(hend, input_shape_data[1]);
                wend = std::min(wend, input_shape_data[2]);
                const int pool_index = ph * output_shape_data[1] + pw;
                for (int h = hstart; h < hend; ++h) {
                  for (int w = wstart; w < wend; ++w) {
                    const int index = h * input_shape_data[2] + w;
            ...
            for (int ph = 0; ph < output_shape_data[0]; ++ph) {
              for (int pw = 0; pw < output_shape_data[1]; ++pw) {
                for (int pz = 0; pz< output_shape_data[2]; ++pz) {
                  ...
                  int wend = std::min(wstart + kernel_shape[1],
                          input_shape_data[2]+ pad_data[1]);
                  int zend = std::min(zstart + kernel_shape[2],
                  ...
                  hend = std::min(hend, input_shape_data[1]);
                  wend = std::min(wend, input_shape_data[2]);
                  zend = std::min(zend, input_shape_data[3]);
