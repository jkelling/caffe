    ...
    for (int di = 0; di < num_datasets; ++di) {
      ...
      if( di > 0) {
        if( shape.size() != dataset_shapes_[di].size()) {
          LOG(FATAL) << filename << " dataset "
                     << this->layer_param_.top(di).c_str() << toString( shape)
                     << " has different number of axes.";
        }
      ...
      msg += std::string("  ") + this->layer_param_.top(di).c_str() + " " +
          toString( shape);
    }
    LOG(INFO) << msg;
    herr_t status = H5Fclose(file_id);
...
template <typename Dtype>
void HDF5DataLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
