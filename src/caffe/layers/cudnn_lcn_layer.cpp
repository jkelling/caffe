...
void CuDNNLCNLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (totalSizeInBytes > tempDataSize) {
    ...
    Caffe::freeGPU(tempData1);
    Caffe::freeGPU(tempData2);

    // allocate new buffers
    CUDA_CHECK(Caffe::mallocGPU(&tempData1, totalSizeInBytes));
    CUDA_CHECK(Caffe::mallocGPU(&tempData2, totalSizeInBytes));
  }
}
